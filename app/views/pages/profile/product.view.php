                    ...
                    <?php foreach ($product['multimedia'] as $index => $file): ?>
                        <article class="py-5 space-y-2">
                            <div class="w-[200px] h-[200px] border border-black rounded-lg shadow-lg flex items-center justify-center overflow-hidden mx-auto">
                                ...
                                <div role="form" class="fetch-form" 
                                onsubmit="useFetch('/product/deleteMultimedia', 'POST', { id : <?= $file['multimedia_id'] ?> } )">
                                    <button type="submit" onclick="confirmAlert(event, '¿Estás seguro que quieres eliminar este archivo?')"
    ...
    function checkFileCount(input, archivos = 7) {
        ...
        if (files.length > (maximo - archivos)) {
            Swal.fire({
                icon: "error",
                title: "Hubo un error",
                text: "Solo puedes seleccionar un máximo de " + (maximo - archivos) + " archivos",
