    ...
    <div class="bg-white p-8 rounded-lg shadow-lg flex flex-col gap-5">
        ...
        <div class="flex flex-col gap-2">
            ...
            <?php if ($order['pedido_estado'] == 'entregado'): ?>
                ...
                <form action="/order/update" method="post" class="fetch-form">

                    <input type="hidden" name="pedido_id" value="<?= $order['pedido_id'] ?>">
                    <input type="hidden" name="pedido_estado" value="recibido">
                    <input type="hidden" name="trabajador_numero_trabajos" value="<?= $order['trabajador_numero_trabajos'] + 1 ?>">
                    <input type="hidden" name="trabajador_saldo" value="<?= $order['trabajador_saldo'] + $order['envio_valor'] ?>">

                    <button onclick="confirmAlert(event, '¿Estás seguro que recibiste el pedido?')"
